...
const Home = () => {
  ...
  const toggleSound = () => {
    const newSound =
      mySound.Sound === "On" ? { Sound: "Off" } : { Sound: "On" };
  ...
  return (
    <div className={homeStyle.startScreen} style={myStyle}>
      ...
      <div className={homeStyle.allBtn} onClick={hideMode}>
        ...
        {showButtons && (
          <div
            className={homeStyle.modeContainer}
            onClick={(event) => event.stopPropagation()}
          >
            <Link to="/player-vs-ai-easy" style={{ textDecoration: "none" }}>
              <button
                className={homeStyle.mode1}
                style={buttonStyle}
                onClick={playClickSound}
              >
                Easy
            ...
            <Link to="/player-vs-ai-hard" style={{ textDecoration: "none" }}>
              <button
                className={homeStyle.mode2}
                style={buttonStyle}
                onClick={playClickSound}
              >
                Hard
        ...
        <div
          className={`${homeStyle.moreContainer} ${showMore ? "show" : "hide"}`}
          onClick={(event) => event.stopPropagation()}
        >
          {showSocial && (
            <button
              className={homeStyle.more1}
              style={buttonStyle}
              onClick={() => {
                revealSocials();
                playClickSound();
          ...
          <div className={homeStyle.settingContainer}>
            <div className="custom-control custom-switch">
              <input
                ...
                checked={mySound.Sound === "On"}
              />
              <label className={"custom-control-label"} htmlFor="soundSwitch">
                {SoundText}
              </label>
            </div>
            <div className="custom-control custom-switch">
        ...
        {showSocials && (
          <div className={homeStyle.socials}>
            <button
              className={homeStyle.socialBtn1}
              style={myStyle}
              onClick={() => {
                window.open("https://www.instagram.com/santoshh689");
                playClickSound();
              }}
            >
            ...
            <button
              className={homeStyle.socialBtn3}
              style={myStyle}
              onClick={() => {
                window.open("https://www.linkedin.com/in/santosh986");
                playClickSound();
              }}
            >
